...
export calcCornerProjectionsAprilTags!
...
# [OPTIONAL TO DO YOURSELF] detect the tags and duplicate the memory before freeing the detector
  # detector = AprilTagDetector()
  # tags = detector.(imgs) .|> deepcopy
  # freeDetector!(detector)

# setup the cost function, you can add more parameters here if you like
obj = (fx, fy, cx, cy) -> _calcCalibResidualAprilTags!( imgs, taglength=taglength, fx=fx, fy=fy, cx=cx, cy=cy, dodraw=false )[1]
obj_ = (fcxy) -> obj(fcxy...)
...
"""
function calcCornerProjectionsAprilTags!( cimg_::AbstractMatrix{<:AbstractRGB}, 
                                          tags_::AbstractVector=begin
                                                                detector = AprilTagDetector()
                                                                __tags__ = detector.(cimg_) .|> deepcopy
                                                                freeDetector!(detector)
                                                                __tags__
                                                              end;
